...
import {MoreVertical, Edit, Trash2, Plus, Car, FolderOpen, Copy} from 'lucide-vue-next';
...
/**
 * Effectue l'appel API de duplication puis recharge les données de la catégorie.
...
const duplicateVehicule = async () => {
  ...
  try {
    await catalogueService.duplicateVehicule(
      catalogue.value.uuid,
      categorie.value.uuid,
      vehiculeToDuplicate.value.uuid
    );
    toast.add({
      severity: 'success',
